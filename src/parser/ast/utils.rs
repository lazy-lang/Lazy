...
pub fn expression_to_string(ast: &ASTExpression, delimiter: Option<char>) -> String {
    ...
    match ast {
        ...
        ASTExpression::Iterator(it) => format!("{}Iterator ({} ..{} {})", unwrapped, expression_to_string(&it.start, delimiter), if it.inclusive { String::from("=" )} else { String::from("") }, expression_to_string(&it.end, delimiter)),
        ...
        ASTExpression::While(while_loop) => format!("{}While {} ({})", unwrapped, expression_to_string(&while_loop.condition, delimiter), expression_to_string(&while_loop.body, delimiter))
